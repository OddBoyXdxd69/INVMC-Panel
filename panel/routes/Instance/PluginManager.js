...
async function getPluginList() {
  try {
    ...
    const response = await axios.get(
      `${API_URL}?size=${ITEMS_PER_PAGE}&page=${page}&sort=-downloads`,
    );
...
async function getPluginVersions(id, minecraftVersion) {
  try {
    ...
    const filteredVersions = pluginVersions.filter((version) =>
      version.game_versions.includes(minecraftVersion),
    );

    if (filteredVersions.length === 0) {
      throw new Error(
        `No compatible versions found for Minecraft version ${minecraftVersion}`,
      );
    }
...
router.get("/instance/:id/plugins/download", async (req, res) => {
  ...
  try {
    ...
    if (downloadResponse.status === 200) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Error downloading plugin." });
    }
  } catch (error) {
    console.error(
      "Error during plugin download:",
