...
router.get("/instance/:id/files/zip/:file", async (req, res) => {
  ...
  try {
    ...
    const apiUrl = `http://${instance.Node.address}:${instance.Node.port}/fs/${instance.VolumeId}/files/zip/${encodeURIComponent(file)}${subPath}`;
    ...
    try {
      ...
      const redirectQuery = filePath
        ...
        : "";
      return res.redirect(
        `/instance/${id}/files?success=ZIPPED${redirectQuery}`,
      );
    } catch (error) {
      console.error(
        `Error zipping file ${file} for instance ${id}:`,
        error.message,
      );

      let errorMessage = "Failed to zip file";
      if (error.response) {
      ...
      const redirectQuery = filePath
        ...
        : "";
      return res.redirect(
        `/instance/${id}/files?err=${encodeURIComponent(errorMessage)}${redirectQuery}`,
      );
    }
  } catch (err) {
    console.error(`Unexpected error in zip route for instance ${id}:`, err);
    const filePath = req.query.path || "";
    const redirectQuery = filePath
      ? `&path=${encodeURIComponent(filePath)}`
      : "";
    return res.redirect(
      `/instance/${id}/files?err=${encodeURIComponent("Internal Server Error")}${redirectQuery}`,
    );
  }
});
